...
Read `SOURCE_AUTHORITY_POLICY.md` first.

- the attached/original target PDF scan is the sole textual authority for canonical Tamil;
- **do not use web search, blogs, Wikisource/Wikipedia, online mirrors, another edition, secondary quotations or parallel publications to resolve source wording**;
- OCR/parsed text from the same controlling scan may only assist drafting;
- if a reading cannot be established confidently from the target scan, retain `[தெளிவில்லை]`;
- outside-source comparison is allowed only if the user explicitly requests it.
